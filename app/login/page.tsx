...
function GoogleLoginButton({ disabled }: { disabled: boolean }) {
  ...
  if (!isClient) {
    return (
      <button
        disabled={true}
        className="w-full py-3 bg-white border border-gray-300 text-gray-700 rounded-lg font-medium disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
      >
  ...
  useEffect(() => {
    ...
    const checkOAuth = async () => {
      try {
        ...
        // Session provider not available (static generation)
      }
    };
